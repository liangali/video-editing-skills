...
# 阶段 2.5: 主题感知片段预选（可选但强烈推荐）
python "<SKILL_DIR>\scripts\select_clips.py" --output-vlm "<WORKSPACE_DIR>\output_vlm.json" --theme "<THEME>" --output "<WORKSPACE_DIR>\candidate_clips.json" [--extra-keywords "关键词1,关键词2"]

# 阶段 3: AI 生成 storyboard.json（通过当前代理可用的文件写入工具写入）
# 若 candidate_clips.json 存在，step 3.6 应以其为片段池，而非重新扫描 output_vlm.json

# 阶段 4: 合成最终视频
"<VENV_PYTHON>" "<SKILL_DIR>\scripts\compose_video.py" --storyboard "<WORKSPACE_DIR>\storyboard.json"
```

**storyboard 硬校验（新增，建议在阶段 3.6 写入后立即执行）：**
...
# 自动修复并回写（按需）
"<VENV_PYTHON>" "<SKILL_DIR>\scripts\storyboard_guard.py" --storyboard "<WORKSPACE_DIR>\storyboard.json" --output-vlm "<WORKSPACE_DIR>\output_vlm.json" --mode autofix --write-back
```
...
> **目的：** 在 AI 创作之前，用 Python 脚本自动完成"哪些视频/片段与主题最相关"的筛选，
> 将 AI 在 step 3.6 的工作量从扫描全量素材压缩为审阅一个精简候选池。

### 步骤 2.5.1 运行 select_clips.py

```bash
python "<SKILL_DIR>\scripts\select_clips.py" \
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `--output-vlm` | — | output_vlm.json 路径（必需） |
| `--theme` | — | 视频主题，与阶段 1 保持一致（必需） |
| `--output` | — | 输出 candidate_clips.json 路径（必需） |
| `--min-videos` | `6` | 主题相关视频不足时，从非相关视频补充至此数量（兜底下限） |
| `--max-per-video` | `2` | 每视频最多保留几个片段；第 1 轮每视频 1 段（广覆盖），第 2 轮补第 2 段 |
| `--min-clip-duration` | `1.5` | 片段最短时长阈值（秒） |
| `--extra-keywords` | 空 | 额外主题关键词，逗号分隔，用于补充评分 |

### 步骤 2.5.2 脚本执行逻辑

脚本内部依次完成以下操作：

1. **主题关键词提取**：从 `--theme` 生成整词 + 2-字 bigram 关键词列表
2. **逐片段评分**：对每个 seg_desc 按**句子级别**检测否定词（如"没有/无法/不符合"等，在同一句话内）；正向命中 +weight，否定命中 -weight；≥3 字关键词权重 ×2
3. **视频排名**：视频总分 = 所有片段得分之和；按总分降序排列
4. **按主题选视频**：选出**全部** video_score > 0 的主题相关视频（越多越好）；若不足 `--min-videos` 则用片段最丰富的剩余视频补充
5. **两轮选片**（每视频最多 `--max-per-video` 段，默认 2）：
   - **第 1 轮**：每个视频取得分最高的 1 段 → 优先保证视频覆盖广度
   - **第 2 轮**：若 `--max-per-video ≥ 2`，再取每个视频第 2 高分段
6. **两轮打散**：第 1 轮所有片段排在前（同轮内按视频得分高→低轮转，相邻不同源），第 2 轮紧随其后，同样轮转打散
7. **写出 candidate_clips.json**：包含 `selection_metadata`（统计摘要）和 `candidate_clips`（已打散的片段列表）

### 步骤 2.5.3 candidate_clips.json 结构

```json
{
  "selection_metadata": {
    "theme": "节日庆典",
    "theme_keywords": ["节日庆典", "节日", "庆典", "节庆", ...],
    "total_videos_in_vlm": 40,
    "theme_matched_videos": 8,
    "selected_videos_count": 6,
    "padded_from_unmatched": 0,
    "total_candidate_clips": 18,
    "selected_video_paths": ["...", "..."],
    "note": "共选出 6 个视频、18 个候选片段..."
  },
  "candidate_clips": [
    {
...
### 步骤 2.5.4 在阶段 3 中使用 candidate_clips.json

若 `<WORKSPACE_DIR>\candidate_clips.json` 存在，**步骤 3.1 应读取 candidate_clips.json**（而非重新扫描 output_vlm.json），将其 `candidate_clips` 列表作为素材池：

- 每个条目的 `seg_desc` 用于步骤 3.1 的素材分类与故事发现
- 步骤 3.6 写 storyboard.json 时，`source_video` / `source_segment_id` / `timecode` 字段直接从对应条目复制，**无需再查 output_vlm.json**
- `seg_desc`、`theme_score`、`video_rank` 等辅助字段**不**写入 storyboard.json

> 注意：candidate_clips.json 是一个"推荐池"，AI 可以根据叙事需要调整顺序或选取子集，但不应从该池以外引入新片段（以免破坏主题一致性）。

** 自检：**  candidate_clips.json 已生成  `total_candidate_clips` ≥ 6  `selected_videos_count` = `--min-videos`（或等于实际视频总数）
...
**计算片段数时须考虑转场折损**：相邻片段间有约 0.8s 转场重叠，每个片段的有效时长 ≈ `seg_duration - transition_duration`（即 3 - 0.8 = 2.2s）。
片段数 ≈ `target / 有效时长`，例如 30s 目标 ≈ 30 / 2.2 ≈ **14 个片段**（不必精确，接近即可）。

| 节拍 | 片段数（30s 约 14 片段） | 情感曲线 | 字幕功能 |
|------|------------------------|---------|---------|
| **开篇点题** | 2-3 个 | 好奇/期待 | 抛出主题，制造悬念或引发共鸣 |
| **铺陈展开** | 3-4 个 | 渐入 | 展开体验，描述出发/过程中的感受 |
| **情感递进** | 4-5 个 | 升温 | 内心感受层层深入，逐步靠近核心情感 |
| **高潮时刻** | 2-3 个 | 最强 | 全篇最有力量的一句话 |
...
- 优先保证来源多样性：最终片段应覆盖**至少 6 个不同的 `source_video`**（若输入视频总数不足 6，则按实际最大数量覆盖并在结果说明）
- 同一 `source_video` 最多使用 3 段（与 `storyboard_guard.py` 校验一致，第 4 段起校验失败）
- 相邻两个 clip 不得来自同一 `source_video`
- 选片顺序优先保证不同 `source_video` 的覆盖率，再按需为各源补第 2、第 3 段（每源不超过 3 段）
...
2. 根据视频氛围，从最匹配的 `style_tag` 分类中选择一首曲目
3. 构建绝对路径：`<SKILL_DIR>\resource\bgm\<file_path>`

**当前 BGM 库以 `bgm/bgm_style.json` 中的 `style_tag` 分类为准**，例如：
...
#### 步骤 3.6.1 运行 storyboard_guard 硬校验（必做）
...
|--------|------|
| `0` | 校验通过，进入阶段 4 |
| 非 `0` | **立即返回步骤 3.2/3.3 重新选片与重排，不得进入阶段 4** |

> 若需要自动修复，可先执行 `--mode autofix --write-back`，再执行一次 `--mode validate`。只有 validate 返回 0 才允许合成。
...
6. `clips` 中不同 `source_video` 的数量应 ≥ 6（若 `<VIDEO_DIR>` 实际视频数不足 6，则按实际数量上限并在结果中说明）
7. 每个 `source_video` 使用次数 ≤ 3（`storyboard_guard.py` 默认 `--per-video-max 3`，超过则校验失败）
8. 任意相邻两个 clip 的 `source_video` 不得相同
9. 每个 clip 的 `voiceover.text` 非空（每个片段必须有字幕）
10. 所有字幕连续读起来是一段完整连贯的叙事（非独立的散句）
11. `file_path` 是绝对路径且 BGM 文件存在
12. `transition.type` 必须是上表中的有效值
13. 最后一个片段不含 transition
14. 粗略估算实际输出时长是否与 target 大致匹配（实际时长 ≈ sum(durations) - 转场重叠总量，偏差合理即可）
...
例如：14 个 3s 片段 + 13 个 0.8s 转场 → 42 - 10.4 = 31.6s ≈ 30s
...
在执行粗检前，必须先运行一次 guard 校验，确保 storyboard 结构和约束合法：

```bash
"<VENV_PYTHON>" "<SKILL_DIR>\scripts\storyboard_guard.py" --storyboard "<WORKSPACE_DIR>\storyboard.json" --output-vlm "<WORKSPACE_DIR>\output_vlm.json" --mode validate
```

若返回非 0，禁止继续阶段 4.1，必须回到阶段 3 调整 storyboard。
...
| `--target-resolution` | 自动 | 不传：对各 `source_video` 探测分辨率后按条数多数决定画幅——竖屏（高>宽）多于横屏 → `1080x1920`，否则（横屏占多数或持平，或无法探测）→ `1920x1080`。传入 `WxH` 则强制统一为该分辨率（保持原始宽高比，不足部分填黑边）。格式非法时脚本会关闭归一化，转场拼接易失败。格式：`WxH`，如 `3840x2160`。需要固定画幅或与自动方向不符时**建议显式传入** |
...
| storyboard 先过 guard | 阶段 4 前必须执行 `storyboard_guard.py --mode validate` 且退出码为 0 |
...
| 最少视频来源覆盖 | clips 需覆盖至少 6 个不同 `source_video`；若输入视频不足 6 个则按实际最大覆盖并说明 |
| 限制重复视频来源 | 同一 `source_video` 最多 3 段（与 guard 默认一致） |
| 禁止相邻复用同一视频 | 相邻两个 clip 不得来自同一 `source_video` |
...
| 路径使用正斜杠 | 始终使用 `\\` 反斜杠 |
| BGM 用相对路径 | 始终拼接绝对路径 |
| 重复使用同一 segment | 写入前检查 `(source_video, source_segment_id)` 唯一 |
| 同一视频被切多段连续使用 | 检查每个 `source_video` 使用次数，并禁止相邻重复 |
