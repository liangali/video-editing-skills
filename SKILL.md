...
| **VLM 模型** | `<SKILL_DIR>\models\Qwen3-VL-8B-Instruct-int4-ov` | OpenVINO GenAI，本地导出（Optimum CLI from HF 模型 `Qwen/Qwen3-VL-8B-Instruct`） |
...
# 阶段 2: 视频分析（始终重新运行）
"<VENV_PYTHON>" "<SKILL_DIR>\scripts\analyze_video.py" --video-dir "<VIDEO_DIR>" --output "<WORKSPACE_DIR>\output_vlm.json" --model-dir "<SKILL_DIR>\models\Qwen3-VL-8B-Instruct-int4-ov" --prompt "<PROMPT>"
...
模型导出由 `setup_ov_model.py` 自动执行，等价核心命令为：

```bash
optimum-cli export openvino --model Qwen/Qwen3-VL-8B-Instruct --task image-text-to-text --weight-format int4 "<SKILL_DIR>\models\Qwen3-VL-8B-Instruct-int4-ov"
```
...
- 检查 / 导出 `<SKILL_DIR>\models\Qwen3-VL-8B-Instruct-int4-ov`（`setup_ov_model.py` 在 `.venv` 内安装导出依赖，并执行 `optimum-cli export openvino`；默认启用 `HF_ENDPOINT=https://hf-mirror.com`；依赖 **OpenVINO 2026.1**）
...
| `--model-dir` | `<SKILL_DIR>/models/Qwen3-VL-8B-Instruct-int4-ov` | 模型目录 |
...
**默认提示词：**
```
按格式输出一行：[镜头:宽景/中景/近景/特写] [光线:明亮/暗淡/逆光/柔和] 然后用30-50字描述主要内容。
```

**需求驱动的提示词模板**（用户指定了任何 theme/mood/pacing/must_capture 时使用）：
```
根据剪辑目标（主题「<THEME>」氛围「<MOOD>」）按格式输出一行：[镜头:宽景/中景/近景/特写] [光线:明亮/暗淡/逆光/柔和] 然后用30-50字描述主要内容，突出与目标主题相关的内容。
```
...
跳过 seg_desc 内容循环重复或完全空洞（< 5 字）的极端异常片段。

#### 2. 素材分级 Triage

根据 seg_desc 的**内容丰富度与视觉/情感价值**，为每个片段打等级：

| 等级 | 判断标准 | 用途 |
|------|---------|------|
| **A 级（精华）** | 描述具体、有画面感，且场景有视觉冲击力或情感价值（如动人风景、有感染力的人物瞬间、强烈运动感、特别的光线） | 优先用于开篇、高潮、结尾 |
| **B 级（可用）** | 描述内容中等，场景普通但可用 | 用于铺陈、过渡 |
| **C 级（跳过）** | 描述循环重复/空洞无物，或模糊抖动无法辨认主体 | 直接排除，不纳入选片 |

**强制规则**：
- 开篇第 1-2 个片段和结尾最后 1-2 个片段**必须从 A 级素材中选取**
- 高潮节拍（情感最强的 2-3 个片段）优先使用 A 级素材
- 若 A 级素材数量不足（< 5 个），放宽标准：将 B 级中内容最丰富的片段升入 A 级，直到 A 级不少于 5 个

> 分级结果在内心建立，无需写入任何文件，供后续步骤 3.2.3 选片时参考。
...
`in_point` 和 `out_point` 可在该 seg_id 的 `[seg_start, seg_end]` 范围内**自由裁剪**，不必照搬 VLM 原始边界。根据叙事节拍调整每个片段的实际使用时长：

| 叙事节拍 | 推荐时长 | 原因 |
|---------|---------|------|
| **开篇**（1-2 片段） | 1.5-2.5s | 快速切入，避免开场拖沓 |
| **铺陈/递进** | 2.5-3.5s | 标准节奏，自然展开 |
| **高潮时刻**（最强 1-2 片段） | 3.5-5.0s | 给最有力量的画面足够的停留时间 |
| **余韵收尾** | 3.5-4.5s | 给观众回味和沉淀的空间 |

裁剪规则：
- 从 seg_start 开始取用时，`in_point = seg_start`，`out_point = seg_start + 目标时长`
- 从 seg 中间取用时，选取画面最稳定、内容最完整的区间
- `duration` 必须始终等于 `out_point - in_point`，且不超过该 seg 的 `seg_end`
- **不允许连续 3 个以上片段时长相同**（防止节奏机械感）
