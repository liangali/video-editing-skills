...
**（必须）设置 HF_ENDPOINT 环境变量"https://hf-mirror.com"**：
...
| 2 | 环境变量 | `HF_ENDPOINT` 已设置则自动使用 |
| 3（默认） | 内置镜像 | 自动使用 `https://hf-mirror.com` |
...
3. **检查模型目录的完整性（执行 FLAMA 前必做）**

   运行以下命令进行完整性校验（`--check-only` 不执行下载）：
   ```powershell
   python "<SKILL_DIR>\scripts\setup_ov_model.py" --flama-dir "<FLAMA_DIR>" --check-only
   ```

   脚本会输出 `.xml`、`.bin` 文件数及总文件数，并给出 ✓ / ✗ 结论。**按以下规则判断：**

   | 脚本输出 | 说明 | 下一步 |
   |---------|------|--------|
   | `✓ 模型目录完整有效` | 通过，可运行 FLAMA | 进入步骤 2.3 |
   | `✗ 模型目录不完整` + 原因（如 `.xml=0`、总文件 < 10） | 目录不存在或下载不完整 | 执行下方步骤 4 重新下载 |

4. **若模型不完整或不存在（必须执行，禁止跳过）**

   **先**创建并激活虚拟环境，**再**安装依赖，**最后**运行下载：
   ```powershell
   # Windows（在 <SKILL_DIR> 下执行）
   python -m venv .venv
   .venv\Scripts\Activate.ps1
   pip install huggingface_hub
   # 正常下载（脚本自动检测并清除不完整目录后重新下载）
   python scripts\setup_ov_model.py --flama-dir "<FLAMA_DIR>"
   # 若上方命令跳过了下载但模型仍不完整，使用 --force 强制重新下载：
   python scripts\setup_ov_model.py --flama-dir "<FLAMA_DIR>" --force
   ```
   - 默认已启用 `hf-mirror.com` 镜像，国内无需额外参数；海外直连时加 `--no-mirror`；自定义镜像用 `--hf-mirror <URL>` 或设置 `HF_ENDPOINT` 环境变量。
   - 脚本会将模型下载到 `<FLAMA_DIR>\models\` 下，并更新 `<FLAMA_DIR>\config.json` 中的 `genai.model_path`。
...
| `--font-file` | 否 | 自动检测系统字体 | 字幕字体文件（不指定时自动查找系统字体） |
| `--font-size` | 否 | `40` | 字幕字号 |
