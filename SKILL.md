...
# 0b. 检查模型路径（必做）：读取 config 的 genai.model_path，若目录不存在则**必须**执行模型转换，不得跳过：
#     (1) 在 <SKILL_DIR> 创建/激活虚拟环境；(2) pip install "optimum-intel[openvino]" nncf qwen-vl-utils；
...
- **需要运行模型转换**（setup_ov_model.py）：**建议使用独立虚拟环境**，避免与系统或其他项目依赖冲突。
...
**安装模型转换依赖**（仅当需要运行 setup_ov_model.py 时执行）：

```bash
pip install "optimum-intel[openvino]" nncf qwen-vl-utils
```

之后在该虚拟环境下执行模型转换（国内推荐加 `--hf-mirror`）：
...
│     视频目录    │────►│ 2.2 检查 config 模型路径 │───►│     大纲        │────►│     compose_    │
│ 1.2 创建        │     │     （不存在→setup_ov_  │    │ 3.2 选择片段    │     │     video.py    │
│     工作区      │     │      model.py）         │    │ 3.3 旁白        │     └─────────────────┘
│ 1.3 保存输入    │     │ 2.3 运行 FLAMA          │    │ 3.4 BGM         │
│ 1.4 提取需求    │     │ 2.4 验证并解析输出      │    │ 3.5 输出 JSON   │
│ 1.5 检查 ffmpeg │     └─────────────────────────┘    └─────────────────┘
└─────────────────┘
```
...
在运行 FLAMA 之前，必须确认其使用的 VLM 模型目录存在；若不存在则**必须立即执行模型转换**，不得跳过、不得先尝试运行 FLAMA、不得用 ffprobe 或手写分镜等方式绕过视频分析。

**硬性规则：模型路径不存在时，必须按顺序执行 (1)→(2)→(3)，不可省略：**
- **(1)** 在 `<SKILL_DIR>` 创建虚拟环境（若尚无 `.venv`）：`python -m venv .venv`，并激活；
- **(2)** 在虚拟环境中安装依赖：`pip install "optimum-intel[openvino]" nncf qwen-vl-utils`；
- **(3)** 运行转换脚本：`python "<SKILL_DIR>\scripts\setup_ov_model.py" --flama-dir "<FLAMA_DIR>"`（国内推荐加 `--hf-mirror`）。
- 转换完成后再次检查模型路径存在，再进入步骤 2.3 运行 FLAMA。
...
4. **若模型路径不存在（必须执行，禁止跳过）**
   - **先**在 `<SKILL_DIR>` 创建并激活虚拟环境，**再**安装依赖，**最后**运行转换：
   ```bash
   ...
   - 脚本会将模型输出到 `<FLAMA_DIR>\models\` 下，并更新 `<FLAMA_DIR>\config.json` 中的 `genai.model_path`。若你读取的是 `flama/config/config.json`，转换完成后可手动将该 config 中的 `model_path` 改为与 FLAMA 实际使用的一致（或使用相对路径 `models/Qwen2.5-VL-7B-Instruct-int4` 并在 FLAMA 运行目录下保留一份 config）。
   - 仅校验不转换（不下载）：`python "<SKILL_DIR>\scripts\setup_ov_model.py" --flama-dir "<FLAMA_DIR>" --check-only`

5. **依赖说明**
   - 运行 `setup_ov_model.py` 前**必须先**在虚拟环境中安装：`pip install "optimum-intel[openvino]" nncf qwen-vl-utils`（参见「环境准备」）。未安装时不得只运行转换脚本。

**✅ 检查点 2.2：** 模型路径指向的目录存在且有效，或已成功执行 `setup_ov_model.py` 并完成转换。
...
| **模型不存在必须或者下载** | 步骤 2.2 发现模型路径不存在时，必须按顺序执行：创建/激活虚拟环境 → 安装 optimum-intel、nncf、qwen-vl-utils → 运行 setup_ov_model.py；禁止跳过转换、禁止先运行 FLAMA、禁止用 ffprobe/手写分镜等方式绕过分析 |
...
| **模型路径不存在** | 模型未转换 | 必须按步骤 2.2 依次执行：创建虚拟环境 → 安装依赖 → 运行 `setup_ov_model.py`，禁止跳过或绕过 |
