...
| `<VENV_PYTHON>` | 统一 Python = `<SKILL_DIR>\.venv\Scripts\python.exe` | `D:\tools\video-editing-skills\.venv\Scripts\python.exe` |
...
#### 选择标准（按优先级，基于 seg_desc 文本判断）

1. **叙事匹配** — seg_desc 描述的内容是否服务于当前故事段落
2. **内容丰富度** — 优先选择 seg_desc 中提及多种元素（人物+场景+动作）的片段
3. **内容多样性** — 避免连续选择 seg_desc 描述相似场景的片段
...
#### 镜头组接规则（基于 seg_desc 文本判断）

- **场景交替**：避免连续 3 个以上片段的 seg_desc 描述同类场景（如连续室内或连续室外）
- **内容节奏**：描述中有动作/运动的片段与描述静态场景的片段交替排列
- **片段时长**：最短 ≥ 1.5s，最长 ≤ 目标时长的 25%（30s vlog 最长单段 ≤ 7.5s）

#### 规则

- **绝不重复**：`(source_video, source_segment_id)` 组合必须唯一，同一段不可复用
- **开场必精彩**：第一个片段应选择 seg_desc 中描述最生动、内容最丰富的片段（如有人物动作、场景变化、光线变化等）
