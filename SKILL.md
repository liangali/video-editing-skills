...
description: "当用户提供视频文件夹要求创建 vlog 剪辑/分镜脚本，或提到 'vlog剪辑'、'视频剪辑脚本'、'video storyboard'、'editing script' 等关键词，或希望从 storyboard.json 渲染最终视频时触发。"
---
...
**✅ 检查点 1→2：** 工作区目录已创建；`<VIDEO_DIR>` 中至少存在 1 个视频文件。
...
**✅ 检查点 2→3：** `output_vlm.json` 文件非空；至少包含 1 个 `processed_videos` 条目且含 `segments` 数据。
