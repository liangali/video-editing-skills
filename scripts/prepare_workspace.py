...
# 代码导读补充：
# - 该脚本是整个工作流的入口之一，负责把“输入视频目录”转成“可执行工作区”。
# - 运行完成后会在工作区写入 runtime_env.json，供后续 analyze/compose 复用。
# - 通过 ffprobe 批量探测视频尺寸并缓存，后续阶段可避免重复探测。
...
def main() -> int:
    ...
    try:
        ...
        effective_skip_model = bool(args.skip_model or lan_enabled)
        if lan_enabled and not args.skip_model:
            print("[准备] 检测到 LAN VLM 配置，阶段 1 自动跳过本机模型准备。")
        ...
        runtime = bootstrap_environment(
            ...
            skip_model=effective_skip_model,
        )
        print("[准备] ✓ 运行时准备完成")
    ...
    try:
        manifest_path = write_runtime_manifest(
            ...
            merge={
                ...
                "vlm_backend": runtime.get("vlm_backend", "openvino"),
                "lan_vlm_base_url": runtime.get("lan_vlm_base_url"),
                "lan_vlm_model": runtime.get("lan_vlm_model"),
            },
        )
